...
import type { Habit, HabitType, HabitReward } from '@/types'
...
export default function HabitModal({ onClose, onSave }: Props) {
  ...
  // Correction : type HabitReward (etait habitreward en minuscule dans les anciens types)
  const [reward, setReward] = useState<HabitReward>(1)
        ...
        <section className="modal-card-body">
          ...
          <div className="field">
            ...
            <div className="control">
              <input className="input" placeholder="Ex: Meditation"
            ...
            <div className="is-flex is-flex-wrap-wrap" style={{ gap: '.5rem' }}>
              {ICONS.map((i) => (
                <button key={i} onClick={() => setIcon(i)}
                  className={`button habit-icon-btn${icon === i ? ' is-active' : ''}`}>
                  {i}
                </button>
              ))}
            </div>
          ...
          {type === 'counter' && (
            <div className="habit-counter-fields">
              ...
              <div className="field">
                <label className="label">Increment rapide</label>
                <div className="control">
          ...
          {/* Recompense — Correction : etait un <ul> orphelin hors de tout field */}
          <div className="field">
            <label className="label">Recompense</label>
            <div className="tabs is-toggle is-fullwidth">
              <ul>
                ...
                <li className={reward === -1 ? 'is-active' : ''}>
                  <a onClick={() => setReward(-1)}>Negatif</a>
                </li>
              </ul>
            </div>
          </div>
