// components/habits/HabitCounterCard.tsx
import type { Habit } from '@/types'

interface Props {
  habit:       Habit
  // Correction : onIncrement appelle maintenant updateCounter (persist Zero-Knowledge)
  onIncrement: (id: string, date: string, value: number) => void
...
export default function HabitCounterCard({ habit, onIncrement, onDelete }: Props) {
  ...
  return (
    <div className={`media habit-card${isDone ? ' done' : ''}`}>
      ...
      {/* Compteur +/- — passe maintenant date en argument */}
        ...
        <div className="field has-addons mb-0">
          <div className="control">
            <button className="button habit-counter__btn"
              onClick={() => onIncrement(habit.id, today, Math.max(0, current - step))}>
              -
            </button>
          </div>
          <div className="control">
          ...
          <div className="control">
            <button className="button habit-counter__btn"
              onClick={() => onIncrement(habit.id, today, current + step)}>
              +
            </button>
          </div>
        </div>
